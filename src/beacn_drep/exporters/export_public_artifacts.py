...
SOUL = WS / 'beacn-drep-soul'
RES = WS / 'beacn-drep-resources'
OUT = CORE / 'data' / 'output' / 'public'
RUNS = CORE / 'data' / 'output'
ACTIONS_CSV = RES / 'data' / 'input' / 'governance' / 'governance_actions_all.csv'
...
def main():
    ...
    actions = load_actions_map()
    rat = load_rationales_latest()

    soul_commit = git_commit(SOUL)
    res_commit = git_commit(RES)
    core_commit = git_commit(CORE)
    ...
    for aid, r in rat.items():
        ...
        (OUT / 'actions' / f'{aid}.md').write_text(Path(CORE / 'data' / 'output' / r['run_id'] / 'rationale.md').read_text())
    ...
    index = {
        ...
        'core': {'repo': 'beacn-drep-core', 'version': '0.1.0', 'commit': core_commit},
        'stats': {
